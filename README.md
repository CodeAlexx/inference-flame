...
| Stable Cascade | SenseNova-U1 8B-MoT | Nucleus-Image 17B MoE |
|---|---|---|
...
| L2P (T2I-L2P, pixel-space) | microsoft/Lens 3.8B (MM-DiT + GPT-OSS-20B MoE) |
|---|---|
| ![L2P](assets/l2p_zidius_eco_fantasy.png) | ![Lens](assets/lens_koi_pond.png) |
| *Tencent T2I-L2P — Z-Image-Turbo DiT body + 16×16 pixel-space patchify + MicroDiffusionModel U-Net head replacing FinalLayer+unpatchify. Direct 1024² pixel-space generation, **no VAE**. 30 steps, CFG 2.0, shift 3.0, seed 42 — 84s on a 24 GB 3090 Ti.* | *microsoft/Lens — 48-block MM-DiT (FLUX.2 VAE, GPT-OSS-20B MoE encoder with MXFP4 experts, YaRN RoPE, sliding-window + attention sinks, token-choice top-4 MoE). 1024², 20 steps, CFG 5.0, sequential encoder→DiT memory strategy — 39 min on a 24 GB 3090 Ti. Per-layer DiT cos ≥ 0.999 vs Python; per-layer encoder cos ≥ 0.999 vs HF.* |

| Boogu-Image-0.1-Base |
|---|
| ![Boogu-Image](docs/boogu_sample.png) |
| *Boogu-Image-0.1-Base — 1024², 20 steps, CFG 4.0, seed 42, pure-Rust. Lumina2/OmniGen2 double+single-stream flow-matching DiT (Qwen3-VL-8B text encoder, FLUX.1-dev VAE). Resident DiT + tiled VAE decode, ~87s on a 24 GB 3090 Ti — **7.6× faster than the Mojo reference port** (2.24 vs 17.06 s/step at 1024²). Parity vs torch oracle: e2e latent cos 0.9965 / image PSNR 31.1 dB.* |

| Ideogram-4 |
|---|
| ![Ideogram-4](docs/ideogram4_sample.png) |
| *Ideogram-4 (`ideogram-ai/ideogram-4-fp8`) — 1024², 20 steps, pure-Rust. 9.3B single-stream sandwich-norm AdaLN flow-matching DiT (head_dim 256, FP8 weight-only per-row, dual conditional/unconditional transformers, Qwen3-VL-8B 13-tap encoder, Flux2 32-ch VAE). 7.4 s/step on a 24 GB 3090 Ti. Parity vs torch oracle: e2e latent cos 0.9917 / image PSNR 33.4 dB. (Photoreal hands + 2D impasto chibi koi — mixed-style prompt adherence.)* |
...
| Boogu-Image-0.1-Base | Lumina2/OmniGen2-lineage flow-matching DiT: 8 double-stream (separate per-stream q/k/v joint attn + img self-attn) + 32 single-stream blocks + 2 context-refiner + 2 noise-refiner, hidden 3360, GQA 28Q:7KV, head_dim 120, 3-axis interleaved RoPE (θ=10000, axes [40,40,40]), RMSNormZero modulation + tanh-gated dual residuals, SwiGLU (inner 13568). Qwen3-VL-8B language-tower text encoder (`hidden_states[-1]`), stock FLUX.1-dev 16ch VAE, FlowMatchEuler + static v1 time-shift (mu 1.15). Ships a Qwen3-VL prompt rewriter (`boogu_rewrite`). | Working — pure-Rust T2I end-to-end, 1024²/20 + CFG 4.0 in ~87s on a 24 GB 3090 Ti (**2.24 s/step**, resident DiT + tiled VAE decode), **7.6× faster than the Mojo reference port** (17.06 s/step). Parity vs torch oracle: full-DiT velocity cos **0.9999189**, e2e latent cos **0.9965** / image PSNR **31.1 dB**; rewriter 100% greedy token-parity vs `transformers .generate`. head_dim 120 hits flame's fused cuDNN SDPA by zero-padding to 128 with a pre-scaled Q (softmax scale stays exactly 1/√120) — ~12× the manual FP32 path; the cuDNN flash kernel never materializes the 4096² score matrix, so the ~20 GB DiT stays resident at 1024². **No flame-core changes** (all ops map to existing primitives). Bins: `boogu_encode` → `boogu_infer` / `boogu_rewrite`. |
| Ideogram-4 (`ideogram-ai/ideogram-4-fp8`) | 9.3B single-stream sandwich-norm AdaLN flow-matching DiT (34 layers, hidden 4608, head_dim **256**, FP8 weight-only per-output-row scale, **two** transformers — conditional + unconditional for asymmetric CFG, 3-axis MRoPE, SwiGLU). Qwen3-VL-8B text encoder (13 hidden-state taps → 53248-dim), Flux2 32-ch VAE, logit-normal flow-match scheduler (hand-rolled `ndtri`). | Working — pure-Rust T2I, parity-verified vs torch oracle (e2e latent cos **0.9917** / image PSNR **33.4 dB**, byte-identical noise). 512²/20 = 1.79 s/step; **1024²/20 = 7.4 s/step** (peak 23.3 GB resident). head_dim 256 is outside cuDNN/WMMA flash {64,96,128} → uses flame-core's materialized SDPA fallback with **Q-tiling** (auto-tiles over `FLAME_SDPA_MATERIALIZE_BUDGET_MB`, default 256 MiB) so the `[L,L]` F32 scores never OOM at 1024² — the one flame-core change (`7b4e281`). Bins: `ideogram4_encode` → `ideogram4_infer`. |
| Klein 4B | Flux 2 DiT (5+20 blocks) | Working |
| Klein 9B | Flux 2 DiT (8+24 blocks) | Working |
| AsymFLUX.2 Klein 9B | Klein 9B base + AsymFlow velocity wrap + Oklab pixel-space VAE replacement (768 = 3 ch × 16² patch) + per-step calibration k = 1/(s + (1 − s)σ) + orthogonal CFG. 58 LoRA pairs fused into FFN/single-block linear2/time-embedder, 3 raw replacements (x_embedder [4096, 768], proj_out [768, 4096], norm_out.linear [8192, 4096]) | Working — 1024²/38 steps in ~115s on a 24 GB 3090 Ti. Pure-Rust pixel-space port of `Lakonik/AsymFLUX.2-klein-9B`, end-to-end (Qwen3-8B text encode → BFL klein9b transformer wrapped with AsymFlow → Oklab decode → PNG). One load-bearing bug found and fixed: BFL klein9b's `final_layer` decodes adaLN output as `[shift, scale]` (first/second halves via `narrow`), diffusers' `AdaLayerNormContinuous` (source of the adapter's `norm_out.linear.weight`) produces `[scale, shift]` — direct copy silently swapped the two roles every inference step, producing a fine-period cross-hatch artifact across the whole image. Fix lives in `src/bin/asymflux2_klein9b_infer.rs`: row-swap the two halves of `norm_out.linear.weight` before insertion. Math primitives in `src/models/asymflux2.rs` (45 unit tests). CPU Oklab in `src/vae/oklab.rs` (22 unit tests). Adapter is **flux-non-commercial-license**. |
...
| Nucleus-Image 17B MoE | 32-block single-stream DiT (3 dense + 29 sparse MoE × 64 experts, expert-choice routing capacity 4 for layers 3-4 / 2 for 5-31, route_scale 2.5, GQA 16Q:4KV, head_dim 128, 3D RoPE [16,56,56] scale_rope, modulation 4-channel (scale1, gate1, scale2, gate2) with `tanh(gate.clamp(-2,2))` residuals, no shift), Qwen3-VL text encoder (text branch only, layer-28 hidden state at `return_index=-8`), Qwen-Image VAE | Working — 1024²/30 + CFG-4.0 in ~6 min on a 24 GB 3090 Ti. Model is 34 GB BF16 (resident OOMs) → `NucleusFacilitator` + `BlockOffloader` streams the ~15.3 GB MoE expert weights from pinned host RAM, ~3.5 GB resident. Per-block text K/V cache amortizes the txt prefix once across all 30 denoise steps. CFG-Zero* per-token norm rescale matches the diffusers pipeline. Three bugs caught by `nucleus_parity` (DiT 1-step parity vs diffusers ground truth: cos > 0.9999): inverted VAE std formula (1/std reassignment), pool-flush at phase boundaries (`FLAME_ALLOC_POOL=0` or explicit `clear_pool_cache()`), and double VAE denormalization (`Wan21VaeDecoder` denormalizes internally; pipeline does too — drop the external pass). |
| microsoft/Lens (Base / RL-tuned / Turbo) | 3.8B 48-block MM-DiT (joint image+text attention, complex-polar 3-axis RoPE, SwiGLU MLP, dual-stream modulation, AdaLayerNormContinuous final) + FLUX.2 VAE (32-ch latent, BatchNorm-normalized) + GPT-OSS-20B MoE text encoder (24 layers, 32 experts top-4, MXFP4-quantized, YaRN RoPE, sliding-window + full alternating, per-head attention sinks, GQA 64Q:8KV). All three checkpoints share identical config — one binary loads any. | Working — pure-Rust end-to-end T2I, 1024² @ 20 steps in ~39 min on a 24 GB 3090 Ti. **5 net-new flame-core kernels shipped for this port**: MXFP4 dequant (`flame_core::ops::fused_inference::dequant_mxfp4_to_bf16`, bit-exact vs `transformers/integrations/mxfp4.py`), YaRN RoPE table builder, sliding-window causal keep-mask, attention sinks SDPA path (`sdpa::forward_with_sinks`), token-choice top-K MoE routing (`ops::token_choice_routing`, dual of Nucleus's expert-choice). Plus a mask-aware Q-tiled materialized SDPA fallback that unblocked masked attention at 1024² on a 24 GB card. Sequential `encoder → drop → DiT` memory strategy fits 24 GB without BlockOffloader (D6 deferred). Per-layer DiT parity vs Python cos ≥ 0.999 (all 20 steps at 512°, 19/20 at 1024°); per-layer encoder parity vs HF `GptOssForCausalLM` cos ≥ 0.999 at all four selected layers (5/11/17/23). One bug found and fixed cross-port: `klein_vae.rs` decoder `bn_eps` was 1e-5 from a stale ERNIE constant, but the FLUX.2 VAE config (which Klein 9B and Lens both load — md5-identical safetensors) ships `batch_norm_eps=1e-4`. The encoder side of the same file already had 1e-4, so the inconsistency was load-bearing only for the decoder path. Empirical impact: ≤ BF16 ULP on running_var ∈ [2.91, 3.47], invisible at runtime but parity-relevant. |
| Helios-Distilled 14B Video DiT | 40-layer single-stream DiT (Wan 2.1 T2V 14B fine-tune), hidden 5120, ffn 13824, head_dim 128, 3D RoPE `[44, 42, 42]`, 6-channel modulation (shift+scale+gate × {self-attn, ffn}) via per-block `scale_shift_table` + temb, FFN GELU-approximate (tanh), `qk_norm="rms_norm_across_heads"` over full inner_dim, `guidance_cross_attn=true` (history split in cross-attn), `zero_history_timestep=true`, multi-term-memory patches (`patch_short`/`mid`/`long` Conv3d for 16+2+1 latent-frame history), 3-stage pyramid sampler with γ=1/3 renoise correction at boundaries, DMD scheduler (`[2,2,2]` steps × 3 stages = 6 denoise calls per chunk), autoregressive 33-frame chunks (Wan VAE 4× temporal stride). UMT5-XXL text encoder, Wan 2.1 VAE. Tiled VAE decode auto-activates above 32×32 latent. | Working — pure-Rust T2V end-to-end, validated against diffusers `HeliosPyramidPipeline` parity. 384×256 @ 132 frames in 458s, 512×320 @ 132 frames in 655s on a 24 GB 3090 Ti. **Known limitations:** (1) per-chunk scene drift in multi-chunk runs (single-chunk `--frames 33` avoids it); (2) distilled-step roughness (only 6 denoise steps); (3) DiT-side OOM at 640×384+ (S≥11k attention sequence — needs chunked self-attention); (4) tiled VAE seam blending may show subtle banding in flat regions; (5) **resolution must be divisible by 64** (latent ÷8 then pyramid ÷4 then patch ÷2); (6) I2V/V2V branches and non-Distilled (Base/Mid) variants not yet wired. The Wan UMT5 attention precision was the load-bearing bug here: Q/K/V must stay BF16 through the QK^T BMM (with cuBLAS F32 accumulator), then scores cast to F32 for softmax — casting Q/K/V to F32 first appears more precise but diverges from torch's BF16-quantized scores at the softmax boundary, drifts invisibly per-layer, then collapses at layer 18. |
