#!/usr/bin/env python
"""Wan2.1 VAE encoder parity test — Python reference.

Loads the same random input that the Rust parity binary generated,
encodes it with the Python Wan2.1 VAE, and compares the output.

Usage:
    # First run the Rust binary to generate the input:
    #   cargo run --release --bin wan21_vae_encode_parity
    # Then run this:
    python scripts/wan21_vae_encode_parity.py
"""
...
INPUT_PATH = f"{OUTPUT_DIR}/wan21_vae_encode_input.safetensors"
RUST_OUTPUT_PATH = f"{OUTPUT_DIR}/wan21_vae_encode_output.safetensors"
PY_OUTPUT_PATH = f"{OUTPUT_DIR}/wan21_vae_encode_output_python.safetensors"

# Add Wan2.2 to path (contains Wan2.1 VAE module)
...
def main() -> int:
    ...
    print("=== Wan2.1 VAE Encoder Parity Test (Python) ===\n")

    # --- Load input from Rust ---
    if not os.path.exists(INPUT_PATH):
        print(f"ERROR: Input file not found: {INPUT_PATH}")
        print("Run the Rust binary first: cargo run --release --bin wan21_vae_encode_parity")
        return 1
    ...
    # --- Load VAE ---
    # The checkpoint might be .safetensors or .pth. Wan2_1_VAE expects a .pth
    # file by default. We need to convert or handle accordingly.
    print("--- Loading Wan2.1 VAE ---")
    t0 = time.time()

    # Check if we have a .pth version
    vae_pth = VAE_PTH
    if vae_pth.endswith(".safetensors"):
        # Try to find a .pth version or convert
        pth_path = vae_pth.replace(".safetensors", ".pth")
        if os.path.exists(pth_path):
            vae_pth = pth_path
        else:
            # Load safetensors and create a temporary state dict
            print(f"  Converting safetensors to state dict...")
            sd = load_file(vae_pth, device=device)
            # Save as temporary .pth
    ...
    with torch.no_grad(), amp.autocast(dtype=dtype):
        # Encode: input is [C, T, H, W] per video, output is [C, T', H', W']
        latent = vae.encode([video.squeeze(0)])[0].float().unsqueeze(0)
