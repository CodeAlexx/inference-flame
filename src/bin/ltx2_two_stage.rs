...
fn main() -> anyhow::Result<()> {
    ...
    // Load per-channel statistics for un-normalize/re-normalize
    ...
    // Un-normalize: x * std + mean (broadcast over [B, C, F, H, W])
    ...
    println!("\n--- Stage 2: Refine at {}×{} ({} steps) ---", TARGET_WIDTH, TARGET_HEIGHT, s2_steps);
    let t0 = Instant::now();
    ...
    // Also add noise to audio for stage 2
