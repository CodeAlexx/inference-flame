...
fn collect_prompt() -> String {
    ...
    while i < args.len() {
        if args[i] == "--prompt" {
            return args.get(i + 1).cloned().unwrap_or_else(|| PROMPT.to_string());
...
fn main() -> anyhow::Result<()> {
    ...
    let (
        ...
        audio_context,
        text_mask,
        neg_video_context_from_text,
        neg_audio_context_from_text,
        neg_mask_from_text,
    ) = if !do_cfg
        && std::path::Path::new(&video_cache).exists()
        && std::path::Path::new(&audio_cache).exists()
        && std::env::var("LTX2_AV_REENCODE").is_err()
    {
        println!("\n--- Stage 1: Text Encoding (cached) ---");
        ...
        let v = vc.get("video_context")
            .ok_or_else(|| anyhow::anyhow!("cached video context missing video_context: {video_cache}"))?
            .to_dtype(DType::BF16)?;
        let a = ac.get("audio_context")
            .ok_or_else(|| anyhow::anyhow!("cached audio context missing audio_context: {audio_cache}"))?
            .to_dtype(DType::BF16)?;
        let m = vc.get("encoder_attention_mask")
            .or_else(|| ac.get("encoder_attention_mask"))
            .ok_or_else(|| anyhow::anyhow!(
                "cached LTX2 contexts missing encoder_attention_mask; set LTX2_AV_REENCODE=1"
            ))?
            .to_dtype(DType::BF16)?;
        println!("  Loaded video_ctx={:?} audio_ctx={:?}", v.dims(), a.dims());
        (v, a, m, None, None, None)
    } else {
        // ========================================
        ...
        // Tokenize (simple: <bos> + text tokens, left-padded to 1024)
        ...
        println!("  Cached contexts: {video_cache} + {audio_cache}");
    ...
    let (neg_video_context_from_text, neg_audio_context_from_text, neg_mask_from_text) = if do_cfg {
        println!("  Tokenizing (negative)...");
        let (neg_ids, neg_mask) = simple_tokenize(&neg_text, TEXT_MAX_LEN)?;
        ...
        let neg_text_mask = feature_extractor::convert_to_additive_mask(&neg_mask_out)?;

        drop(neg_hidden);
        drop(neg_mask_out);
        (Some(v_neg), Some(a_neg), Some(neg_text_mask))
    } else {
        (None, None, None)
    };
    ...
    // When LoRAs are attached, avoid FP8-resident — that path can't re-fuse
    // deltas after dequant and would silently miss audio LoRAs. The BF16
    // streaming offloader still fuses LoRAs per block.
    if fp8_stream {
        ...
        if !lora_specs.is_empty() {
            println!("  LoRA attached — skipping FP8 resident, using BF16 streaming BlockOffloader");
            ...
            match model.load_fp8_resident() {
                ...
                Err(e) => {
                    println!("  FP8 resident failed ({e}), falling back to BF16 streaming BlockOffloader");
                    model.init_offloader_streaming()?;
                    println!("  Streaming BlockOffloader initialized in {:.1}s", t0.elapsed().as_secs_f32());
                }
            }
        }
    } else if pinned_offloader {
        println!("  Using pinned BF16 BlockOffloader (--pinned-offloader)");
        model.init_offloader()?;
        println!("  Pinned BlockOffloader initialized in {:.1}s", t0.elapsed().as_secs_f32());
    } else {
        println!("  Using BF16 streaming BlockOffloader (--pinned-offloader opts into old eager pinning)");
        model.init_offloader_streaming()?;
        println!("  Streaming BlockOffloader initialized in {:.1}s", t0.elapsed().as_secs_f32());
    }
    ...
    //      and is a bug.
    let (neg_video_context, neg_audio_context, neg_context_mask) = if do_cfg {
        let nvc_path: Option<String> = get_arg("--neg-video-embeddings");
        let nac_path: Option<String> = get_arg("--neg-audio-embeddings");
        let loaded_neg = nvc_path.is_some() || nac_path.is_some();
        ...
        let nmask = if loaded_neg {
            println!("  Loaded negative embeddings have no saved text mask; treating all positions as real");
            None
        } else {
            neg_mask_from_text.clone()
        };
        (Some(nvc), Some(nac), nmask)
    } else {
        (None, None, None)
    };
    ...
    for step in 0..num_steps {
        ...
        let (video_vel, audio_vel) = if do_cfg {
            ...
            let (mut video_uncond, mut audio_uncond) = model.forward_audio_video(
                ...
                nvc, nac,
                FRAME_RATE,
                neg_context_mask.as_ref(), neg_context_mask.as_ref(),
            )?;
            if cfg_star {
