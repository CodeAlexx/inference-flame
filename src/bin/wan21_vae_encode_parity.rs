//! Wan2.1 VAE encoder parity test.
//!
//! Loads the Wan2.1 (Anima) VAE encoder, encodes a small random tensor,
...
use inference_flame::vae::Wan21VaeEncoder;

const VAE_PATH: &str = "/home/alex/.serenity/models/anima/split_files/vae/qwen_image_vae.safetensors";
...
fn main() -> anyhow::Result<()> {
    ...
    println!("=== Wan2.1 VAE Encoder Parity Test ===\n");

    // --- Create random input [1, 3, 1, 256, 256] in [-1, 1] ---
    // Single frame for Anima (image VAE).
    ...
    let input_path = format!("{OUTPUT_DIR}/wan21_vae_encode_input.safetensors");
    ...
    // --- Load encoder ---
    println!("--- Loading Wan21 VAE Encoder ---");
    let t0 = Instant::now();
    ...
    // Save output
    let output_path = format!("{OUTPUT_DIR}/wan21_vae_encode_output.safetensors");
    ...
    println!("  python scripts/wan21_vae_encode_parity.py");
