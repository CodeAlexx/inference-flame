//! Klein 9B (base) image generation — Turbo Flame Phase 1 (VMM-backed
//! double-buffered block loader).
//!
//! Same pipeline as `klein9b_infer` but the per-block weight staging goes
//! through `inference_flame::turbo::TurboBlockLoader` instead of the
//! in-process CPU staging dictionary.
//!
//! Behind feature `turbo`. On a non-VMM-capable device this binary logs
//! a hint and exits with code 2 — no silent fallback.
...
const OUTPUT_PATH: &str = "/home/alex/EriDiffusion/inference-flame/output/klein9b_rust_turbo.png";
...
fn main() -> anyhow::Result<()> {
    ...
    // ------------------------------------------------------------------
    // Stage 1: Encode text (Qwen3 8B) — then drop
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    println!("\n--- Stage 2: Load Klein 9B (Turbo / VMM) ---");
    let t0 = Instant::now();

    // NonBlocking stream, one-shot ordered against device.cu_stream() at fork;
    // concurrent thereafter. H2D prefetch here runs in parallel with compute
    // kernels on device.cu_stream(). (cudarc StreamKind::NonBlocking).
    ...
    let arena = match VmmArena::new_for_klein9b(device.clone(), copy_stream.clone()) {
        Ok(a) => Arc::new(a),
        Err(VmmError::Unsupported) => {
            log::error!(
                "VMM unsupported on device {}, re-run with klein9b_infer (non-turbo)",
                device.ordinal()
            );
            std::process::exit(2);
        }
    ...
    // Block prefixes in the canonical order: doubles 0..N then singles 0..M.
    let mut block_prefixes: Vec<String> = Vec::with_capacity(cfg.num_double + cfg.num_single);
    for i in 0..cfg.num_double {
        block_prefixes.push(format!("double_blocks.{i}."));
    }
    for i in 0..cfg.num_single {
        block_prefixes.push(format!("single_blocks.{i}."));
    }
    ...
    .map_err(|e| anyhow::anyhow!("TurboBlockLoader: {e}"))?;
