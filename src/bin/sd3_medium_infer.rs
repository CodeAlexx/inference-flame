//! SD3.5 Medium end-to-end inference — pure Rust, flame-core.
//!
//! Full pipeline: prompt → CLIP-L + CLIP-G + T5-XXL → DiT denoise → VAE → PNG.
//! Model fits resident in 24GB. No block swapping needed.
//!
//! Usage: sd3_medium_infer ["prompt text"]
...
const EMBEDDINGS_PATH: &str = "/home/alex/EriDiffusion/inference-flame/output/sd3_medium_embeddings.safetensors";
...
/// CLIP tokenize (L or G — same vocab). Pad to 77 with EOS (49407).
    ...
    match tokenizers::Tokenizer::from_file(tokenizer_path) {
        Ok(tok) => {
            ...
            while ids.len() < CLIP_SEQ_LEN {
                ids.push(49407); // EOS = pad
            }
            ids
        }
        Err(e) => {
            eprintln!("[sd3] CLIP tokenizer failed: {e}; using BOS+EOS fallback");
            let mut ids = vec![49406i32, 49407]; // BOS, EOS
...
/// T5-XXL tokenize. Pad to T5_SEQ_LEN with 0.
...
/// Encode cond + uncond through all 3 encoders in one pass per encoder.
/// Returns (cond_hidden, cond_pooled, uncond_hidden, uncond_pooled).
///
/// Each encoder is loaded once, encodes both prompts, then dropped before
/// the next encoder loads — so peak GPU is never more than one encoder at a time.
fn encode_text_pair(
    prompt: &str,
    ...
    // Helper: load CLIP weights as BF16
    ...
    let (clip_l_hidden, clip_l_pooled, clip_l_hidden_u, clip_l_pooled_u) = {
        ...
        let tokens_c = tokenize_clip(prompt, CLIP_L_TOKENIZER);
        let tokens_u = tokenize_clip(empty, CLIP_L_TOKENIZER);
        let (hc, pc) = clip.encode_sd3(&tokens_c)?;
        let (hu, pu) = clip.encode_sd3(&tokens_u)?;
        (hc, pc, hu, pu)
        // `clip` dropped here — weights freed
    };
    ...
    let (clip_g_hidden, clip_g_pooled, clip_g_hidden_u, clip_g_pooled_u) = {
        ...
        let tokens_c = tokenize_clip(prompt, CLIP_G_TOKENIZER);
        let tokens_u = tokenize_clip(empty, CLIP_G_TOKENIZER);
        let (hc, pc) = clip.encode_sd3(&tokens_c)?;
        let (hu, pu) = clip.encode_sd3(&tokens_u)?;
        (hc, pc, hu, pu)
    };
    ...
    // ---- T5-XXL (cond + uncond, then drop) ----
    println!("  T5-XXL...");
    let t0 = Instant::now();
    let (t5_hidden, t5_hidden_u) = {
        let mut t5 = T5Encoder::load(T5_PATH, device)?;
        let tokens_c = tokenize_t5(prompt);
        let tokens_u = tokenize_t5(empty);
        let hc = t5.encode(&tokens_c)?;
        let hu = t5.encode(&tokens_u)?;
        (hc, hu)
        // t5 dropped here — 9GB freed
    };
    // Truncate to T5_SEQ_LEN (T5 encoder pads to its internal max of 512)
...
/// Zero-pad the last dimension of a tensor: [B, N, C] → [B, N, target_dim].
...
fn main() -> anyhow::Result<()> {
    ...
    println!("============================================================");
    println!("SD3.5 Medium — Pure Rust End-to-End Inference");
    println!("  Prompt: {:?}", prompt);
    println!("  {}x{}, {} steps, CFG {}, shift {}", WIDTH, HEIGHT, NUM_STEPS, CFG_SCALE, SHIFT);
    println!("============================================================");

    // ------------------------------------------------------------------
    // Stage 1: Text encoding (CLIP-L + CLIP-G + T5-XXL)
    // Each encoder loaded once, encodes cond+uncond, then freed.
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // Stage 2: Load SD3.5 Medium (all resident)
    // ------------------------------------------------------------------
    println!("\n--- Stage 2: Load SD3.5 Medium ---");
    let t0 = Instant::now();
    let resident = load_sd3_all(MODEL_PATH, &device)?;
    println!("  {} resident keys", resident.len());
    ...
    let denoised = inference_flame::sampling::klein_sampling::euler_denoise(
        |x, t_curr| {
            // SD3 model expects timesteps in [0, 1000] range (sinusoidal embedding).
            // The schedule produces sigmas in [0, 1]; multiply by 1000.
    ...
    // Free DiT + embeddings before VAE to avoid OOM at 1024×1024
    drop(model);
    drop(context);
    drop(pooled);
    drop(context_uncond);
